...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!orderCode) return json({ ok: true, note: "no orderCode" }); // αγνόησε άσχετα events

    const { data: order } = await db.from("orders")
      .select("*").eq("viva_order_code", orderCode).single();
    if (!order) return json({ ok: true, note: "order not found" });
    if (order.status === "paid") return json({ ok: true, note: "already paid" }); // idempotent

    // Επαλήθευση από το Viva (όχι εμπιστοσύνη στο payload)
    const state = await viva.orderState(orderCode);
    if (!state.paid) return json({ ok: true, note: `state ${state.stateId}` });

    // Δεδομένα θεάματος + items
    ...
    await db.from("orders").update({
      status: "paid", paid_at: new Date().toISOString(), viva_state_id: state.stateId,
    }).eq("id", order.id);

    // 2) θέσεις → sold (online) & καθάρισμα holds
    const seatIds = (items ?? []).map((i: any) => i.seat_id).filter(Boolean);
    if (seatIds.length) {
      await db.from("seats").update({ status: "sold", sold_channel: "online" }).in("id", seatIds);
    }
    ...
    for (let k = 0; k < (tickets ?? []).length; k++) {
      ...
      const view = {
        venueName: show.venue_name || "", showTitle: show.title,
        showSubtitle: show.subtitle || "", date: fmtDateGr(show.show_date),
        time: show.start_time || "", seat: it?.seats?.seat_label || "—",
      ...
      const url = `${base}/functions/v1/ticket?uid=${tk.serial_uid}`;
      await db.from("tickets").update({ ticket_url: url }).eq("id", tk.id);
      if (k === 0) htmlInline = await renderTicketHtml(view); // πρώτο εισιτήριο inline
      pdfTickets.push(view);
    }

    // 4) email (Resend) — best effort, δεν μπλοκάρει την έκδοση
    ...
    if (resendKey) {
      ...
      } catch (_e) { /* αν αποτύχει το PDF, στέλνουμε μόνο HTML+link */ }

      const links = (tickets ?? [])
        .map((t: any) => `<li><a href="${base}/functions/v1/ticket?uid=${t.serial_uid}">${t.serial}</a></li>`).join("");
      const body = `<div style="font-family:sans-serif">
        <h2>Η κράτησή σας επιβεβαιώθηκε</h2>
        <p>${show.title} — ${fmtDateGr(show.show_date)} ${show.start_time}</p>
        <p>Τα εισιτήριά σας:</p><ul>${links}</ul>
        <p>Το εισιτήριο είναι και συνημμένο σε PDF.</p>
        <hr>${htmlInline}
      </div>`;
      ...
      try {
        await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            subject: `Εισιτήρια — ${show.title}`, html: body, attachments,
          }),
        });
      } catch (_e) { /* log only */ }
    }
